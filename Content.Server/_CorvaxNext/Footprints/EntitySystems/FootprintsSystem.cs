...
public sealed class FootprintsSystem : EntitySystem
{
    ...
    private const float FootprintDecayTime = 30.0f; // 30 seconds to fully decay
    private readonly Dictionary<EntityUid, TimeSpan> _footprintCreationTimes = new();
    ...
    private void OnMove(EntityUid uid, FootprintVisualizerComponent component, ref MoveEvent args)
    {
        ...
        var spawnPosition = CalcCoords(gridUid, component, transform, dragging);
        var entity = Spawn(component.StepProtoId, spawnPosition);
        ...
        // Track creation time for decay
        _footprintCreationTimes[entity] = IoCManager.Resolve<IGameTiming>().CurTime;
        ...
        _solution.TryAddReagent(solutionEntity.Value, component.ReagentToTransfer, 0.8, out _);
    }
    ...
    public override void Update(float frameTime)
    {
        ...
        foreach (var (footprint, creationTime) in _footprintCreationTimes)
        {
            var age = (currentTime - creationTime).TotalSeconds;

            if (age >= FootprintDecayTime)
            {
                // Time to remove this footprint
            ...
            {
                var fadeProgress = (float)(age / FootprintDecayTime);
                var currentAlpha = 0.7f * (1.0f - fadeProgress); // Start at 0.7, fade to 0

                if (_appearance.TryGetData<Color>(footprint, FootprintVisualState.Color, out var currentColor, appearance))
                {
                    var newColor = currentColor.WithAlpha(currentAlpha);
                    _appearance.SetData(footprint, FootprintVisualState.Color, newColor, appearance);
                }
            }
        }

        // Remove expired footprints
        foreach (var footprint in toRemove)
        {
            _footprintCreationTimes.Remove(footprint);
            if (Exists(footprint))
            {
                Del(footprint);
            }
